...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (shiftResult.rowCount === 0) throw new Error("Brew Houze isn't taking orders right now. Please order again once the café is open.");
    ...
    const order = await client.query(`
      INSERT INTO sales_orders (cashier_admin_id, total_amount, status, queue_number, queue_status, order_source, customer_order_token, payment_method, received_amount, change_amount, shift_id)
      VALUES (NULL, $1, 'completed', $2, 'waiting', 'online', $3, 'online', $1, 0, $4)
    ...
    `, [total, queueNumber, customerToken, shiftId]);
    ...
    for (const variant of variants.rows) {
      ...
      `, [order.rows[0].order_id, variant.product_id, variant.product_variant_id, group.quantity, variant.price, variantCosts.get(Number(variant.product_variant_id)) ?? null]).then(async (itemResult) => {
